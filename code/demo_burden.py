...
def calculate_burden_with_variance(df):
    ...
    mit_mean = pot_mean - obs_mean
    mit_var = pot_var + obs_var

    df["obs_mean"], df["obs_var"] = obs_mean, obs_var
    df["pot_mean"], df["pot_var"] = pot_mean, pot_var
    df["mit_mean"], df["mit_var"] = mit_mean, mit_var
...
for p, col_name in zip(["obs", "pot", "mit"], ["observed", "potential", "mitigated"]):
...
keep_columns = [
    "NAME", "year", "month",
    "observed_total", "potential_total", "mitigated_total",
    "observed_per100k", "potential_per100k", "mitigated_per100k"
]
